  ...
  <div v-if="visible" class="large-modal" @click.self="closeModal">
    <div class="large-modal-content">
      <div class="modal-header">
        <h3>Edit Optical Distribution Point (ODP)</h3>
        ...
        <div class="modal-body">
          ...
          <div class="form-row-grid">
            ...
            <div class="input-group">
              ...
              <select v-model="formTipePemasangan">
                <option value="tiang">Tiang (Pole)</option>
                <option value="tanam">Tanam (Pedestal)</option>
              </select>
            </div>
          </div>
          ...
          <div class="form-row-grid">
            <div class="input-group">
              ...
              <select v-model="formOdcId">
                <option :value="null">-- Tidak terhubung ke ODC --</option>
                <option v-for="odc in store.odcList" :key="odc.id" :value="odc.id">
                  {{ odc.nama }}
                </option>
              </select>
            </div>
      ...
      <div v-else class="modal-body" style="gap: 16px">
        ...
        >
          <p style="margin: 0; font-size: 14px; color: var(--sb-text-primary)">
            Anda harus login untuk mengedit ODP. Silakan kembali dan login terlebih dahulu.
          </p>
        </div>
      </div>
    </div>
  ...
  <Teleport to="body">
    ...
    >
      <div class="large-modal-content" style="max-width: 450px">
        <div class="modal-header">
          <h3 style="color: #ef4444">Konfirmasi Hapus ODP</h3>
...
async function saveChanges() {
  ...
  try {
    const res = await fetch(`/api/odp/${props.device.data.id}`, {
      method: "PUT",
    ...
    if (res.ok) {
      await store.loadAll();
      const updated = store.odpList.find((x) => x.id === props.device.data.id);
      if (updated) {
        store.setSelectedDevice({ type: "odp", data: updated });
      }
      success("Perubahan ODP berhasil disimpan!");
      closeModal();
    } else {
...
async function confirmDelete() {
  ...
  try {
    ...
    if (res.ok) {
      ...
      success("ODP berhasil dihapus!");
    } else {
      const data = await res.json();
      error(data.error || "Gagal menghapus ODP.");
    }
  } catch (err) {
