  ...
  <div id="btn-login-container">
    ...
    >
      <img
        ...
        alt="Login"
        style="width: 16px; vertical-align: middle; filter: invert(0.9)"
      />
      Login
    </button>
    ...
    >
      <img
        src="/icons/user-round-check.svg"
        alt="User"
        style="width: 16px; height: 16px; filter: invert(0.9)"
      />
      <span>{{ auth.username }}</span>
      <img
        src="/icons/chevron-down.svg"
        alt="Chevron"
        style="width: 12px; height: 12px; filter: invert(0.7); margin-left: 2px"
      />
    </button>
    ...
    <div v-if="auth.isLoggedIn && dropdownOpen" class="user-dropdown" @click.stop>
      <div class="dropdown-item" id="dropdown-username">
        <img
          src="/icons/user-round-check.svg"
          alt="User"
          style="width: 16px; height: 16px; filter: invert(0.7)"
        />
        <span>{{ auth.username }}</span>
      </div>
...
const emit = defineEmits(["open-login", "open-settings", "open-panduan"]);
