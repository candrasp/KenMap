...
export const FTTH_ICONS = {
  'sto': L.icon({ iconUrl: '/icons/sto.svg', iconSize: [28, 28], iconAnchor: [14, 14], popupAnchor: [0, -10] }),
  'olt': L.icon({ iconUrl: '/icons/olt.svg', iconSize: [28, 28], iconAnchor: [14, 14], popupAnchor: [0, -10] }),
  'odc': L.icon({ iconUrl: '/icons/odc.svg', iconSize: [28, 28], iconAnchor: [14, 14], popupAnchor: [0, -10] }),
  'odp-tanam': L.icon({ iconUrl: '/icons/odp-tanam.svg', iconSize: [28, 28], iconAnchor: [14, 14], popupAnchor: [0, -10] }),
  'odp-tiang': L.icon({ iconUrl: '/icons/odp-tiang.svg', iconSize: [28, 28], iconAnchor: [14, 14], popupAnchor: [0, -10] }),
  'klien-aktif': L.icon({ iconUrl: '/icons/klien-aktif.svg', iconSize: [24, 24], iconAnchor: [12, 12], popupAnchor: [0, -8] }),
  'klien-nonaktif': L.icon({ iconUrl: '/icons/klien-nonaktif.svg', iconSize: [24, 24], iconAnchor: [12, 12], popupAnchor: [0, -8] }),
  'klien-pending': L.icon({ iconUrl: '/icons/klien-pending.svg', iconSize: [24, 24], iconAnchor: [12, 12], popupAnchor: [0, -8] }),
}
...
export const useInfrastrukturStore = defineStore('infrastruktur', () => {
  ...
  function setMeasuring(val) {
    ...
    const groups = [stoLayerGroup, oltLayerGroup, odcLayerGroup, odpLayerGroup, klienLayerGroup, kabelLayerGroup]
  ...
  function renderKabel() {
    kabelList.value.forEach((kabel) => {
      ...
      if (kabel.jenis === 'feeder') style = { color: '#10b981', weight: 4.5, opacity: 0.9, interactive: !isMeasuring.value }
      else if (kabel.jenis === 'drop') style = { color: '#f59e0b', weight: 2, opacity: 0.8, dashArray: '5, 5', interactive: !isMeasuring.value }

      const jenisLabel = { feeder: 'Kabel Feeder', distribusi: 'Kabel Distribusi', drop: 'Kabel Drop' }[kabel.jenis] || 'Kabel'
  ...
  function renderSto() {
    stoList.value.forEach((sto) => {
      ...
      const marker = L.marker([sto.lat, sto.lng], { icon: FTTH_ICONS['sto'], interactive: !isMeasuring.value })
      marker.on('click', () => {
        if (isMeasuring.value) return
        setSelectedDevice({ type: 'sto', data: sto })
        const offset = window.innerWidth < 768 ? [0, -(window.innerHeight * 0.25)] : [0, 0]
        marker._map?.setView(marker.getLatLng(), marker._map.getZoom(), { animate: true, pan: { offset } })
      })
      marker.bindTooltip(sto.nama, { permanent: true, direction: 'right', offset: [12, 0], className: 'device-label' })
  ...
  function renderOlt() {
    oltList.value.forEach((olt) => {
      ...
      const marker = L.marker([olt.lat, olt.lng], { icon: FTTH_ICONS['olt'], interactive: !isMeasuring.value })
      marker.on('click', () => {
        if (isMeasuring.value) return
      ...
      marker.bindTooltip(olt.nama, { permanent: true, direction: 'right', offset: [12, 0], className: 'device-label' })
  ...
  function renderOdc() {
    odcList.value.forEach((odc) => {
      ...
      const marker = L.marker([odc.lat, odc.lng], { icon: FTTH_ICONS['odc'], interactive: !isMeasuring.value })
      marker.on('click', () => {
        if (isMeasuring.value) return
      ...
      marker.bindTooltip(odc.nama, { permanent: true, direction: 'right', offset: [12, 0], className: 'device-label' })
  ...
  function renderOdp() {
    odpList.value.forEach((odp) => {
      ...
      const marker = L.marker([odp.lat, odp.lng], { icon: FTTH_ICONS[iconKey], interactive: !isMeasuring.value })
      marker.on('click', () => {
        if (isMeasuring.value) return
      ...
      marker.bindTooltip(odp.nama, { permanent: true, direction: 'right', offset: [12, 0], className: 'device-label' })
  ...
  function renderKlien() {
    ...
    klienList.value.forEach((klien) => {
      ...
      const marker = L.marker([klien.lat, klien.lng], { icon: FTTH_ICONS[iconKey], interactive: !isMeasuring.value })
      marker.on('click', () => {
        if (isMeasuring.value) return
      ...
      marker.bindTooltip(klien.nama, { permanent: false, direction: 'top', offset: [0, -14], className: 'device-label', sticky: false })
      klienLayerGroup?.addLayer(marker)

      // Draw a dashed connection line from klien to its connected ODP
      if (klien.odp_id) {
        const odp = odpMap.get(klien.odp_id)
        if (odp?.lat && odp?.lng) {
          const statusColorMap = { aktif: '#10b981', nonaktif: '#ef4444', pending: '#f59e0b' }
          const lineColor = statusColorMap[klien.status] || '#f59e0b'
          const dropLine = L.polyline(
            [[klien.lat, klien.lng], [odp.lat, odp.lng]],
            {
              color: lineColor,
              weight: 3,
              opacity: 0.6,
  ...
  return {
    stoList, odcList, odpList, klienList, kabelList, oltList, isMeasuring, selectedDevice,
    setLayerGroups, setMeasuring, setSelectedDevice, clearSelectedDevice, loadAll, renderAll,
  }
})
